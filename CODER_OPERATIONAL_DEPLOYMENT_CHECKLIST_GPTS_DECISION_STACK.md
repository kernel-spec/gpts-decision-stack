...
- **A-03** The `promotion-gate.yaml` `current_gate_position` section reflects a snapshot in time. Its `overall_status: FAIL` reflects the state before full production evidence was assembled. The `qa/final-gate-report.yaml` is the authoritative gate record; consult it, not `promotion-gate.yaml` snapshot fields, for current truth.
...
- [ ] `actions/openapi.yaml` is the **action contract surface**. All GPT actions must match the routes, methods, and auth patterns defined here.
...
Confirm each of the following action routes has a concrete handler implementation (not a stub returning 200 with no logic):
...
## 6. API/Auth Contract Checks

These checks confirm that the deployed Worker's behavior matches the contract declared in `actions/openapi.yaml` and the auth model in `actions/auth.md`.

### 6.1 OpenAPI Consistency
- [ ] Every route implemented in `backend/worker/src/router.ts` has a corresponding path in `actions/openapi.yaml`
- [ ] Every path in `actions/openapi.yaml` has a corresponding implemented handler in `backend/worker/src/handlers/`
- [ ] HTTP methods declared in `actions/openapi.yaml` match the methods implemented in the router
- [ ] Request and response body shapes in `actions/openapi.yaml` match the TypeScript types in `backend/worker/src/types/index.ts`
- [ ] `actions/openapi.openai.yaml` is consistent with `actions/openapi.yaml` — no path, method, or schema divergence

### 6.2 Auth Model Consistency
- [ ] `actions/auth.md` describes the `X-API-Key` / `Authorization` authentication pattern actually implemented in the Worker
- [ ] `GET /health` is a public endpoint (no auth required) — confirm this matches `actions/openapi.yaml` and the handler
- [ ] All non-health endpoints require `X-API-Key` or equivalent auth header — confirm this matches `actions/auth.md`
...
### 6.3 Evidence Script to Worker Auth Alignment
...
- [ ] `operations/gates/promotion-gate.yaml` `current_gate_position` section is updated to reflect the current state (not left at a historical snapshot)
...
A handoff package is required before this deployment is considered complete. The handoff package enables a different Coder to take over implementation state without hidden knowledge.
...
- [ ] `actions/openapi.yaml` defines all required routes
...
- [ ] `operations/gates/promotion-gate.yaml` `current_gate_position` section reflects current actual state
...
- [ ] Handoff package is assembled per Section 12 and committed to the repository
