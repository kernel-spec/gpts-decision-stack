...
Pilot baseline je formálně uzavřená. Na main jsou nyní:
...
To znamená, že další krok už není pilot merge práce, ale stabilizace deployment convention
před prvním skutečným operator runem.
...
## Kratší operátorská verze

**Builder Convention Hardening**

Cíl: Zpevnit convention pro custom GPT vrstvu bez repo-wide rewritu.

Potvrdit:
- `*.full.md` = canonical source-of-truth
- `*.builder.md` = deployment form
- pilot YAML = evidence, ne prompt source
- summary docs = summary, ne canonical truth
- operator-run docs = samostatná operating evidence vrstva

Review rules:
- evidence PR odděleně
- prompt PR odděleně
- summary PR odděleně
- operator-run docs odděleně
- actual files > preview glitches

Out-of-scope:
- dashboard
- repo-wide prompt rewrite
- backend/workflows/tests/gates

Immediate next step: spustit OPRUN-001 přes SYSTEM_OS_MASTER → POSITIONING_POLICE
